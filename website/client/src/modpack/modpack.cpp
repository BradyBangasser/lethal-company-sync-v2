...
int ModPack::install() {
    if (isInstalled() == Status::INSTALLED) {
        ...
        std::array<std::thread, numberOfInstallerThreads> downloadThreads;
        std::array<std::thread, numberOfInstallerThreads> installerThreads;

        for (i = 0; i < numberOfInstallerThreads; i++) {
